...
with tab_2:
    ...
    st.markdown(""" Regardons à présent plus en détail le f1-score pour chacun des modèles:
                """)
    ...
    sns.barplot(ax = axes[0], data = df_report_bis, x = 'Modele', y = 'f1_macro_avg', hue = 'Evaluation', palette= ['lightgrey', 'darkgrey'])
    axes[0].set_title("Moyenne f1")
            
    sns.barplot(ax = axes[1],data = df_report_bis, x = 'Modele', y = 'f1_0', hue = 'Evaluation', palette=['palegoldenrod', 'gold'])
    axes[1].set_title("f1_0 sur la classe négative: pas de pluie")

    sns.barplot(ax = axes[2],data = df_report_bis, x = 'Modele', y = 'f1_1', hue = 'Evaluation', palette=['paleturquoise', 'darkturquoise'])
...
with tab_3:
    ...
    ## == SOUS-SECTION ==
    st.header("Réduire l'overfitting sur RdF pour réduire l'écart de qualité des prédictions?")

    st.markdown(""" Tentons de réduire l'overfitting sur RdF en faisant de la feature selection.
                """)
    feature_importance = pd.read_csv("../../models/saves/reports/rdf_feats_importances.csv").sort_values(by = 'Importance', ascending = False)
    ...
    st.markdown("""Pour cela, on enlève au fur et à mesure les features de moindre importances 
